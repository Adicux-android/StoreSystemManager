...
public class DateUtils {
    ...
    public static String getCurrentTime()
    {
        ...
        String day = time.monthDay > 9 ? "" + time.monthDay : "0" + time.monthDay;
        String hour = (time.hour + 8) > 9 ? "" + (time.hour + 8) : "0" + (time.hour + 8);
        String minute = time.minute > 9 ? "" + time.minute : "0" + time.minute;
        String sec = time.second > 9 ? "" + time.second : "0" + time.second;
        String currentTime = year + month + day + hour + minute + sec; return currentTime; }
    //获取 日期/时/分/秒
    ...
    public static String getHourMinute(long time)
    {
        ...
        String date = sdf.format(new Date(time));
        return date; }
    //获取 日期年月日 @SuppressLint("SimpleDateFormat")
    ...
    public static String getYearMonth2(long time)
    {
        ...
        String date = sdf.format(new Date(time)); return date; }
    //获取日期年
